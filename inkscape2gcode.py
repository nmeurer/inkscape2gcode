...
class LaserGcode(inkex.Effect):
	...
	def __init__(self):
		...
		self.arguments = [
			...
			{"name": "--x-offset", "type": str, "dest": "x_offset", "default": 0,
			 "help": "X-Offset"},

			{"name": "--y-offset", "type": str, "dest": "y_offset", "default": 0,
			 "help": "Y-Offset"},
			
			{"name": "--z-offset", "type": str, "dest": "z_offset", "default": 0,
			 "help": "Z-Offset"},
			...
			for i in range(6):
				if c[i] != None:
					...
					#Handling Y Offset
						...
						r += s[i] + ("%f" % (round(c[i], 4) + float(self.options.y_offset))).rstrip('0')
					#Handling Z Offset
					elif s[i] == " Z":
						r += s[i] + ("%f" % (round(c[i], 4) + float(self.options.z_offset))).rstrip('0')
					else:
